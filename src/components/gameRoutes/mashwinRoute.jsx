...
const writeMessage = () => {
if (index + 1 < dialog.length) { setIndex(index + 1); setText(dialog[index + 1]); setDisplayText(''); } };
  useEffect(()=> {
  let i = 0;
  ...
  <section className="h-screen overflow-hidden bg-black farm-font">
    <div className={`grid place-content-center h-screen ${showScene} ${showScene===scene
      ...
      <div className={`relative mt-64 animate__animated animate__fadeInUp animate__slow `}>
        {dialogBoxEmpty && <> <img src={DialogBoxNoChar} alt="dialogBox" className=' w-[26rem] relative' />
          ...
          <p
            className=' absolute max-w-[12.25rem] top-2  bottom-0 text-sm text-[#531A12] md:text-4xl md:left-6 md:top-4 md:max-w-[26rem]'>
            {displayText}</p>

          {goBackButton && <button onClick={()=>{
            playSound(SmallSelect)
            navigate('/')
            }} className='absolute left-0 right-0 bottom-4 text-2xl text-[#531A12] hover:border-2 border-red-700 w-fit
            px-10 mx-auto '>Ir al menu</button>}
        ...
        <div>
          ...
          <> <img src={DialogBox} alt="dialogBox" className='w-full max-w-3xl relative ' />
            <p
              className=' absolute max-w-[12.25rem] top-2 left-4 bottom-0 text-sm text-[#531A12] md:text-3xl md:left-6 md:top-4 md:max-w-[26rem]'>
              {displayText}</p>

            {index === 11 ? <img src={Bear}
              className='absolute top-6 right-16 w-28 md:w-[160px] md:top-10 md:right-[5rem]' alt="Oso" /> : <img
              src={Ruma} className='absolute top-6 right-16 w-7 md:w-12 md:top-12 md:right-[8.3rem]' alt="Ruma" /> }
            <p
              className='absolute right-[4.75rem] bottom-[2.225rem] text-sm text-[#531A12] font-semibold md:text-3xl md:right-32 md:bottom-6'>
              {index === 11 ? 'Oso' : 'Ruma'}</p>
            {index + 1 < dialog.length && showButton && <button
              className='absolute cursorLoad w-8 h-4 top-[76%]  animate-pulse bottom-0 right-[42%]'
              onClick={handleClick}><img src={dialogButton} className='absolute  hover:scale-110'
                alt="dialog button box" /></button>
              }
            </>
            }
        </div>
